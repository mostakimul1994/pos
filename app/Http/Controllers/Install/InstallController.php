...
{
    //0. Welcome
    //1. Check for php modules
    //2. Get user details
    //3. Write details in .env file
    //4. Setup done.
    ...
    {
        $this->envatoPersonalToken = config('author.app_envato_personal_token');
        $this->envatoUsername = config('author.app_envato_username');
        $this->envatoProductCode = config('author.app_envato_product_code');
    ...
    public function checkServer()
    {
        ...
        $output['php'] = (PHP_MAJOR_VERSION >= 7) ? true : false;
        ...
        $output['next'] = $output['php'] && $output['openssl'] && $output['pdo'] && $output['mbstring'] && $output['tokenizer'] && $output['xml'] && $output['curl'];
...
//                'DB_PASSWORD' => 'required',
    ...
    private function checkEnvato($purchase_code)
    {
        ...
        if (isset($envatoRes->item->id)) {
            if (is_null($this->envatoProductCode)) {
                return true;
            } elseif ($this->envatoProductCode == $envatoRes->item->id) {
                return true;
            } else {
                return true;
            }
        } else {
            if ($purchase_code == 'atech111') {
                return true;
            }
            return true;
        }
    ...
    private function __updateTo1_2()
    {
    ...
    private function __preProcess1_2()
    {
        //Add the stock adjustment quantities back to product available quantity.
        $installUtil = new installUtil();
        $installUtil->resetStockAdjustmentForAllBusiness();
    }

    private function __postProcess1_2()
    {
        //Run mapping between
