  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="{{ $request->segment(1) == 'home' ? 'active' : '' }} aaa">
          <a href="/pos/public/home">
        ...
        @if(auth()->user()->can('product.view') || auth()->user()->can('product.create') )
          <li class="treeview {{ in_array($request->segment(1), ['variation-templates', 'products', 'labels', 'import-products', 'import-opening-stock']) ? 'active active-sub' : '' }}" id="tour_step5">
        ...
        <li class="treeview {{ $request->segment(1) == 'purchases' ? 'active active-sub' : '' }}" id="tour_step6">
        ...
        @if(auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') )
          <li class="treeview {{  in_array( $request->segment(1), ['sells', 'pos']) ? 'active active-sub' : '' }}" id="tour_step7">
        ...
        @if(auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') )
          <li class="treeview {{  in_array( $request->segment(1), ['sell-return']) ? 'active active-sub' : '' }}">
            <a href="#" id="tour_step7_menu"><i class="fa fa-exclamation-triangle"></i> 
            ...
            <ul class="treeview-menu">
              @can('sell.view')
                <li class="{{ $request->segment(1) == 'sell-return' && $request->segment(2) == null ? 'active' : '' }}" ><a href="{{action('SellReturnController@index')}}"><i class="fa fa-list"></i>@lang('lang_v1.list_sell_return')</a></li>
              @endcan
              @can('sell.create')
                <li class="{{ $request->segment(1) == 'sell-return' && $request->segment(2) == 'create' ? 'active' : '' }}"><a href="{{action('SellReturnController@create')}}"><i class="fa fa-plus-circle"></i>@lang('lang_v1.sell_return')</a></li>
              @endcan
            </ul>
          </li>
        @endif
        ...
        @if(in_array('tables', $enabled_modules) && in_array('service_staff', $enabled_modules) )
          ...
          <li class="{{ $request->segment(1) == 'bookings'? 'active' : '' }}">
              <a href="{{action('Restaurant\BookingController@index')}}"><i class="fa fa-calendar-check-o"></i> @lang('restaurant.bookings')</a>
          </li>
          @endif
        @endif

        @if(in_array('kitchen', $enabled_modules))
          <li class="{{ $request->segment(1) == 'modules' && $request->segment(2) == 'kitchen' ? 'active' : '' }}">
              <a href="{{action('Restaurant\KitchenController@index')}}"><i class="fa fa-fire"></i> @lang('restaurant.kitchen')</a>
          </li>
        @endif
        @if(in_array('service_staff', $enabled_modules))
          <li class="{{ $request->segment(1) == 'modules' && $request->segment(2) == 'orders' ? 'active' : '' }}">
              <a href="{{action('Restaurant\OrderController@index')}}"><i class="fa fa-list-alt"></i> @lang('restaurant.orders')</a>
          </li>
        @endif
        ...
        <li class="treeview @if( in_array($request->segment(1), ['business', 'brands', 'tax-rates', 
        'units', 'categories', 'barcodes', 'invoice-schemes', 'business-location', 'invoice-layouts', 'printers', 'system-settings', 'payment-account', 'subscription']) || in_array($request->segment(2), ['tables', 'modifiers']) ) {{'active active-sub'}} @endif">
          ...
          <ul class="treeview-menu" id="tour_step3">
            ...
            <!-- payment account -->
            @can('payment_account.access')
              <li class="hide @if( in_array($request->segment(1), ['payment-account']) ) {{'active'}} @endif">
                <a href="{{action('PaymentAccountController@index')}}"><i class="fa fa-money"></i> <span>@lang('lang_v1.payment_account')</span></a>
              </li>
            @endcan
